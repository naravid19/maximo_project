...
from maximo_app.models import Site
...
class Command(BaseCommand):
    help = 'นำเข้าข้อมูลจากไฟล์ Excel สำหรับ SITE'
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            # อ่านข้อมูลจาก Excel
            df = pd.read_excel(excel_file_path, sheet_name='SITE')
        ...
        required_columns = ['SITEID', 'DESC', 'ORGID']
            ...
            with transaction.atomic():
                ...
                for index, row in df.iterrows():
                    ...
                    if len(row['DESC']) > 100:
                        ...
                        continue
                    if len(row['ORGID']) > 8:
                        logger.warning(f"Skipping row {index+1}: ORGID exceeds maximum length (8 characters)")
                        self.stdout.write(self.style.WARNING(f"Skipping row {index+1}: ORGID exceeds maximum length (8 characters)"))
                        continue

                    # ข้ามแถวที่ข้อมูลไม่สมบูรณ์
                    if pd.isna(row['SITEID']) or row['SITEID'] == '' or pd.isna(row['ORGID']) or row['ORGID'] == '':
                    ...
                    if created:
                        logger.info(f"Site {site.site_id} added")
                        self.stdout.write(self.style.SUCCESS(f"Site {site.site_id} added"))
                    else:
                        # แสดงคำเตือนถ้าข้อมูลมีอยู่แล้ว
                        logger.warning(f"Site {site.site_id} already exists")
                        self.stdout.write(self.style.WARNING(f"Site {site.site_id} already exists"))
